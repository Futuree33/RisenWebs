<script setup>
  import { ref } from 'vue'

  const headerColour = ref("transparent")

  window.addEventListener("scroll", () => {
    headerColour.value = window.scrollY === 0 ? "transparent" : "rgba(255, 255, 255, 0.05)"
  })
</script>

<template>
    <header :style="{background: headerColour}" class="fixed backdrop-blur transition duration-200 ease-linear z-[100] w-full flex px-5 items-center py-5 lg:px-[100px]">
        <div class="flex-[1]">
            <h1 class="font-semibold text-[20px]">
                WebWorks
            </h1>
        </div>

        <nav class="flex-[3] hidden lg:flex items-center justify-center gap-10">
            <RouterLink to="/" class="router-link active:text-[#524cff_!important] transition duration-200">Home</RouterLink>
            <RouterLink to="/contact" class="router-link active:text-[#524cff_!important] transition duration-200">Contact Us</RouterLink>
        </nav>

        <div class="flex-[1] flex lg:justify-center justify-end">
            <RouterLink class="border-2 flex items-center gap-4 bg-transparent border-[#504CFF] text-[15px] transition duration-300 px-5 py-3.5 rounded-full" to="">
                Get Started
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="1" y="1" width="20" height="20" rx="10" stroke="white"/>
                  <path d="M15.0211 13.7727L14.798 8.10815C14.7915 8.00261 14.7643 7.89775 14.7179 7.79958C14.629 7.60182 14.4682 7.44104 14.2705 7.35212C14.1723 7.30574 14.0675 7.27855 13.9619 7.27209L8.29738 7.04892C8.19175 7.04476 8.08797 7.06145 7.99198 7.09803C7.89598 7.1346 7.80964 7.19036 7.73789 7.26211C7.59299 7.40701 7.5163 7.60827 7.52471 7.8216C7.53311 8.03493 7.62592 8.24286 7.78271 8.39965C7.9395 8.55644 8.14743 8.64925 8.36076 8.65765L12.0935 8.79906L7.41933 13.4732C7.27502 13.6175 7.19864 13.8179 7.20701 14.0304C7.21539 14.2429 7.30781 14.4499 7.46396 14.6061C7.62012 14.7623 7.8272 14.8547 8.03966 14.863C8.25212 14.8714 8.45256 14.795 8.59687 14.6507L13.271 9.9766L13.4124 13.7093C13.4161 13.815 13.4408 13.9206 13.4849 14.02C13.529 14.1194 13.5918 14.2106 13.6695 14.2883C13.7472 14.366 13.8384 14.4287 13.9378 14.4729C14.0371 14.517 14.1427 14.5416 14.2485 14.5454C14.3542 14.55 14.4582 14.5336 14.5544 14.4972C14.6506 14.4607 14.7371 14.405 14.809 14.3332C14.8808 14.2613 14.9365 14.1748 14.9729 14.0786C15.0094 13.9824 15.0257 13.8785 15.0211 13.7727Z" fill="white"/>
                </svg>
            </RouterLink>
        </div>
    </header>

    <router-view />
</template>

<style scoped>
.router-link {
  position: relative;
  text-decoration: none;
  color: inherit;
  margin: 0 10px;
  display: inline-block;
}

.router-link::after {
  content: '';
  position: absolute;
  width: 100%;
  transform: scaleX(0);
  height: 2px;
  bottom: -5px;
  left: 0;
  background-color: white; /* Customize the color as needed */
  transform-origin: bottom right;
  transition: transform 0.25s ease-out;
}

.router-link:hover::after {
  transform: scaleX(1);
  transform-origin: bottom left;
}

</style>